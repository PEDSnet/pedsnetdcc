...
@click.option('--copy', is_flag=True, default=False,
              help='Copy results to output.')
@click.option('--skip_query', is_flag=True, default=False,
              help='Skip actual query.')
@click.option('--noidx', is_flag=True, default=False,
              help='skip indexes.')
@click.option('--nofk', is_flag=True, default=False,
              help='skip FKs.')
...
@click.option('--owner', required=False, default='loading_user',
              help='the role that permissions should be granted to if permissions limited')
@click.argument('dburi')
def run_r_obs_recover(pwprompt, searchpath, site, model_version, idname, copy, noidx, nofk, skip_query, limit, owner, dburi):
    """Run R Script.
    ...
    success = run_r_obs_recover(conn_str, site, password, searchpath, model_version, idname,
                              copy, noidx, nofk, skip_query, limit, owner)
...
@click.option('--source_schema', required=True,
              help='Schema where the source (adult + pediatric) tables are located')
@click.option('--target_schema', required=True,
              help='Schema where the subset tables (adult only) should be located')
@click.argument('dburi')
def run_r_pcornet_adult_slice(pwprompt, searchpath, site, source_schema, target_schema, dburi):
    """Run R Script.
    ...
    conn_str = make_conn_str(dburi, searchpath, password)
    config_file = site + "_pcornet_adult_slice_argos_temp.json"

    from pedsnetdcc.r_pcornet_adult_slice import run_r_pcornet_adult_slice
    success = run_r_pcornet_adult_slice(config_file, conn_str, site, password, source_schema, target_schema)
...
@click.option('--site', required=True,
              help='PCORnet site name for the config file.')
@click.option('--source_schema', required=True,
              help='Schema where the source (PCORnet tables are located')
@click.option('--target_schema', required=True,
              help='Schema where the subset tables (COVID only) should be located')
@click.argument('dburi')
def run_r_pcornet_covid_slice(pwprompt, searchpath, site, source_schema, target_schema, dburi):
    """Run R Script.
    ...
    conn_str = make_conn_str(dburi, searchpath, password)
    config_file = site + "_pcornet_covid_slice_argos_temp.json"

    from pedsnetdcc.r_pcornet_covid_slice import run_r_pcornet_covid_slice
    success = run_r_pcornet_covid_slice(config_file, conn_str, site, password, source_schema, target_schema)
...
def subset_by_cohort(searchpath, pwprompt, dburi, model_version, force, source_schema, target_schema, cohort_table,
    ...
    if split_measure or pre_split:
        from pedsnetdcc.subset_by_cohort import conn_str_with_search_path
...
@click.option('--cohort_table', required=True,
              help='Name of the cohort table where the person_ids are located')
@click.argument('dburi')
def subset_pcornet_by_cohort(searchpath, pwprompt, dburi, model_version, force, source_schema, target_schema, cohort_table,
                     inc_hash, index_create, fk_create, notable, nopk, limit, owner):
    """Create tables for subset based on a cohort/person_id table
    ...
    from pedsnetdcc.subset_pcornet_by_cohort import run_subset_pcornet_by_cohort
    success = run_subset_pcornet_by_cohort(conn_str, model_version, source_schema, target_schema, cohort_table,
                         inc_hash, index_create, fk_create, notable, nopk, limit, owner, force)
